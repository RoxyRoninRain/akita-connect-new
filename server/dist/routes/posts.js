...
router.get('/', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        // 1. Fetch Posts
        const { data: posts, error } = yield supabase
            .from('posts')
            .select(`
                *,
        ...
        const [commentsResult, likesResult] = yield Promise.all([
        ...
        (_a = commentsResult.data) === null || _a === void 0 ? void 0 : _a.forEach(c => {
            const list = commentsMap.get(c.post_id) || [];
            list.push(c);
            commentsMap.set(c.post_id, list);
        });
        (_b = likesResult.data) === null || _b === void 0 ? void 0 : _b.forEach(l => {
            const list = likesMap.get(l.post_id) || [];
            list.push(l.user_id);
            likesMap.set(l.post_id, list);
        });
        // 3. Attach data
        const populatedPosts = posts.map(post => (Object.assign(Object.assign({}, post), { comments: commentsMap.get(post.id) || [], likes: likesMap.get(post.id) || [] })));
...
router.post('/:id/comment', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        // Get post author
        const { data: post } = yield supabase
            .from('posts')
            .select('author_id')
            .eq('id', id)
            .single();
        if (post && post.author_id !== author_id) {
            // Get commenter name
            const { data: commenter } = yield supabase
                .from('profiles')
                .select('name')
                .eq('id', author_id)
                .single();
            yield supabase.from('notifications').insert({
                user_id: post.author_id,
                type: 'comment',
                title: 'New Comment',
                message: `${(commenter === null || commenter === void 0 ? void 0 : commenter.name) || 'Someone'} commented on your post`,
...
router.post('/:id/like', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const { data: existing } = yield supabase
        ...
        .single();
    if (existing) {
        yield supabase.from('likes').delete().eq('user_id', user_id).eq('post_id', id);
        res.json({ liked: false });
    }
    else {
        yield supabase.from('likes').insert([{ user_id, post_id: id }]);
        // Create Notification
        try {
            // Get post author
            const { data: post } = yield supabase
            ...
            if (post && post.author_id !== user_id) {
                // Get liker name
                const { data: liker } = yield supabase
                    .from('profiles')
                    .select('name')
                    .eq('id', user_id)
                    .single();
                yield supabase.from('notifications').insert({
                    ...
                    type: 'like',
                    title: 'New Like',
                    message: `${(liker === null || liker === void 0 ? void 0 : liker.name) || 'Someone'} liked your post`,
