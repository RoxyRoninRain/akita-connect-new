...
const seedData = async () => {
    ...
    for (const akita of akitas) {
        const { error } = await supabase.from('akitas').upsert(akita);
        if (error) console.error('Error creating akita:', akita.call_name, error.message);
        else console.log('Created akita:', akita.call_name);
    }
    ...
    for (const litter of litters) {
        const { error } = await supabase.from('litters').insert(litter);
        if (error) console.error('Error creating litter:', error.message);
        else console.log('Created litter');
    }
